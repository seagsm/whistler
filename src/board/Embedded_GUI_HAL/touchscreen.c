...
__IO FLASH_Status TSC_FlashStatus;
__IO Flash_TestStatus TSC_MemoryProgramStatus;
...
void TS_Calibration(void)
{
  ...
  GL_Clear(GL_White);
  GL_DisplayAdjStringLine(3 * (LCD_Height / 7), LCD_Width - 25, "Run Calibration.", FALSE);
  GL_Delay(40);
  GL_DisplayAdjStringLine(3 * (LCD_Height / 7), LCD_Width - 25, "Run Calibration..", FALSE);
  GL_Delay(40);
  GL_DisplayAdjStringLine(3 * (LCD_Height / 7), LCD_Width - 25, "Run Calibration...", FALSE);
  ...
  GL_Clear(White);
  GL_DisplayAdjStringLine(3 * (LCD_Height / 7), 10 * (LCD_Width / 11), "Calibration done!", FALSE);
...
static void TS_SaveCalibrationVariables(void)
{
  ...
  TSC_FlashStatus = FLASH_COMPLETE;
