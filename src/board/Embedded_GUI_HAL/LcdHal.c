...
static void BmpBuffeR112BitRead(uint32_t* ptR112BitBuffer, uint8_t* ptrBitmap);
...
/**
  ...
  * @param  bool Trasparent_Flag, if TRUE it does not print the GL_BackColor
  * @retval None
  */
void GL_LCD_DisplayChar(uint16_t Line, uint16_t Column, uint8_t Ascii, bool Transparent_Flag)
{
  Ascii -= 32;
  switch (GL_Font)
  {
...
void GL_LCD_DrawCircle(uint8_t Xpos, uint16_t Ypos, uint16_t Radius)
{
  ...
  while (CurX <= CurY)
  {
   // LCD_SetCursor(Xpos + CurX, Ypos + CurY);
    LCD_PutPixel(Xpos + CurX, Ypos + CurY, GL_TextColor, SinglePixel);
   ...
   // LCD_SetCursor(Xpos + CurX, Ypos - CurY);
    ...
    LCD_PutPixel(Xpos + CurY, Ypos + CurX, GL_TextColor, SinglePixel);
...
void GL_DrawBMP(uint8_t* ptrBitmap)
{
  ...
  BmpBuffeR112BitRead(&uBmpSize, ptrBitmap + 2);
  BmpBuffeR112BitRead(&uDataAddr, ptrBitmap + 10);

  /* Set GRAM write direction and BGR = 1 */
  /* I/D=00 (Horizontal : decrement, Vertical : decrement) */
  /* AM=1 (address is updated in vertical writing direction) */
  LCD_WriteReg(R11, 0x6008);
...
{
  if ((PixelSpec & (FirstPixel | SinglePixel)) != RESET)
    LCD_SetPixel(Xpos, Ypos, Color);
}
...
/**
  ...
  * @param  ptR112BitBuffer - 32Bit buffer to fill
  * @retval None
  */
void BmpBuffeR112BitRead(uint32_t* ptR112BitBuffer, uint8_t* ptrBitmap)
{
  *ptR112BitBuffer = 0;
  *ptR112BitBuffer = (*ptrBitmap);
  *ptR112BitBuffer += (*(ptrBitmap + 1)) << 8;
  *ptR112BitBuffer += (*(ptrBitmap + 2)) << 16;
  *ptR112BitBuffer += (*(ptrBitmap + 3)) << 24;
}
